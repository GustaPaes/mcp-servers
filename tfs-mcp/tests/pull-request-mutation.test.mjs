...
test("reports a sanitized partial result when PR metadata succeeds but linking fails", async () => {
  fs.rmSync(auditPath, { force: true });
  const previousFetch = globalThis.fetch;
  globalThis.fetch = async (url, options = {}) => {
    const requestUrl = String(url);
    const method = options.method ?? "GET";
    if (method === "POST" && requestUrl.includes("/pullrequests")) {
      return jsonResponse({
        pullRequestId: 43,
        title: "corrige retorno parcial dos itens relacionados",
        artifactId:
          "vstfs:///Git/PullRequestId/11111111-1111-1111-1111-111111111111%2F22222222-2222-2222-2222-222222222222%2F43",
        repository: {
          id: "22222222-2222-2222-2222-222222222222",
          name: "example-repo",
          project: { id: "11111111-1111-1111-1111-111111111111" },
    ...
    if (method === "GET" && requestUrl.includes("/wit/workitems/303")) {
      return jsonResponse({ message: "accessToken=secret-value internal payload" }, 403);
    }
    return jsonResponse({ message: "unexpected synthetic request" }, 500);
  };
  ...
  try {
    const result = await toolCreatePR({
      source_branch: "feature/exemplo",
      target_branch: "develop",
      titulo: "corrige retorno parcial dos itens relacionados",
      resumo: "corrige de forma segura o retorno parcial dos itens relacionados",
      work_item_ids: [303],
      dry_run: false,
      confirm: true,
      reason: "validar retorno parcial sanitizado",
      requestedBy: "teste automatizado",
    });

    assert.equal(result.pullRequestId, 43);
    assert.equal(result.partial, true);
    assert.deepEqual(result.warnings, ["1 vínculo(s) de work item não foram aplicados."]);
    assert.deepEqual(result.workItemLinks.failed, [
      { id: 303, error: "A solicitação ao TFS falhou com status 403." },
    ]);
    assert.doesNotMatch(JSON.stringify(result), /secret-value|internal payload/);
    ...
    const auditEntries = fs
      ...
      .split(/\r?\n/)
      .map((line) => JSON.parse(line));
    assert.equal(auditEntries.at(-1).status, "applied");
    assert.equal(auditEntries.at(-1).result.partial, true);
  } finally {
    globalThis.fetch = previousFetch;
    fs.rmSync(auditPath, { force: true });
  }
