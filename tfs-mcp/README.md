...
The server supports reusable enterprise TFS installations. Versioned documentation
and examples use synthetic values; keep installation-specific configuration,
scripts and operational data under `local-private/`.
...
When `work_item_ids` are supplied, create and update operations use one
revision-guarded `ArtifactLink` reconciliation path. They do not also submit a
second PR-link mutation, which avoids duplicate relations during eventual
consistency. The response reports `partial:true`, a sanitized warning and the
per-item `workItemLinks.failed` list when PR metadata succeeds but one or more
links cannot be applied.
